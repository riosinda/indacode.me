...
export default function ProjectCard({ name, description, techStack, githubUrl }: ProjectCardProps) {
  return (
    <div className="group border border-border rounded-lg p-5 hover:border-accent/40 transition-colors flex flex-col gap-3">
      ...
      {techStack.length > 0 && (
        <div className="flex flex-wrap gap-2">
          {techStack.map((tech) => (
            <span
              key={tech}
              className="text-xs text-text-secondary border border-border px-2 py-0.5 rounded"
            >
              {tech}
            </span>
          ))}
